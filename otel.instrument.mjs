// OpenTelemetry bootstrap for the Nitro backend (телеметрия). Loaded via `NODE_OPTIONS=--import
...
// runs exactly as before. Turn it on by pointing at a collector (see docs/PROJECT_MAP.md).
//
// PRIVACY (docs/PROJECT_MAP.md): a redaction SpanProcessor strips sensitive auto-instrumentation
...
if (!enabled) {
  ...
  const resource = resourceFromAttributes({
    [ATTR_SERVICE_NAME]: process.env.OTEL_SERVICE_NAME || 'ai-price-import-backend',
  ...
  const sdk = new NodeSDK({
    ...
    instrumentations: [getNodeAutoInstrumentations({
      ...
      // (which could be supplier/article/price) are never captured. db.statement is still redacted above.
      '@opentelemetry/instrumentation-pg': { enhancedDatabaseReporting: false }
    })]
  })

  sdk.start()
  console.info('[otel] started → %s (service=%s)', endpoint, process.env.OTEL_SERVICE_NAME || 'ai-price-import-backend')
