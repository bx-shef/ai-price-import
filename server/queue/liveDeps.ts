...
export function liveCrmSyncDeps(memberId: string, jobId: string, mapping: PortalMapping, rest: (m: string) => Promise<SdkTransport | null>, infra: LiveInfra): CrmSyncDeps {
  ...
  // из Postgres И зовёт `app.info`. Без мемо правка #385 добавляла второй SELECT и второй REST на
  // каждый отказ, ничего ими не узнавая: домен один и тот же, портал тот же, обработка та же.
  // `undefined` — «ещё не спрашивали», `null` — «спросили, адреса нет»; второе тоже кэшируется,
  // иначе мёртвая база опрашивалась бы повторно ровно тогда, когда она мертва.
  // ⚠ Источников отказа теперь ДВА, и кэшируются они одинаково: нет домена (постоянно) и не ответил
  // `app.info` — вне контекста приложения, отказ в правах, сетевой сбой (может быть временным).
  // Различать их в пределах ОДНОГО задания незачем: адрес нужен максимум дважды и подряд, а повтор
  // упавшего вызова стоил бы ещё одного обращения к порталу ради ссылки, без которой сообщение
  // прекрасно доедет. Следующее задание спросит заново — мемо живёт ровно столько же, сколько оно.
