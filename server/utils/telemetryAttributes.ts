// Privacy policy for OpenTelemetry span attributes (телеметрия, docs/PROCESS.md §11 «Телеметрия»).
...
// URLs with tokens) are redacted by a span processor.
//
 ...
 * operation (which method, which queue, outcome, timing/counts) — never its business content
 * (supplier, article, price, document body). A hashed portal id (`portal.hash`) lets us
 * correlate per-portal without exposing member_id.
 */
export const SAFE_MANUAL_ATTR_KEYS = new Set<string>([
  // dependency call (B24 REST)
  'dep.system', // 'bitrix24'
  ...
  // queue / job
  'job.queue', // 'crm-sync' | 'file-extract' | 'agent-run'
  ...
  'proc.unmatched', // boolean: supplier company could not be matched
  'proc.idempotent', // boolean: redelivery of an already-processed job
  'proc.warnings', // number of warnings
  'proc.errors', // number of errors
  // correlation
  'portal.hash'
])

/** Keep only allowlisted keys whose value is a safe primitive. Drops everything else
 *  (unknown keys, objects, arrays, null/undefined) so a caller can't accidentally attach a payload. */
 ...
 * params; `*.url`/`*.query`/`*.target` can carry access tokens (our OAuth/refresh URLs);
 * `*.body`/`*.message` can carry payloads. Belt-and-suspenders: the collector also filters,
 * but we scrub at the source so PII never leaves the process.
 */
