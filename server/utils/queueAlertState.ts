...
// In-process holder for the queue-health check (BACKLOG.md §1). Deliberately NOT in Postgres or
// Redis: the check runs on the cron instance, which is the same process that serves `/api/ops/*`
// (the `worker` role runs with QUEUE_CRON=0 and is not reachable through nginx), and the verdict is
// derived fresh from live queue state every tick — there is nothing worth surviving a restart.
//
