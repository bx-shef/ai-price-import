...
/**
 * Same text with BB-markup stripped — for channels that don't parse it.
 *
 * Тот же текст без BB-разметки — для каналов, которые её НЕ разбирают.
 *
 * ЗАЧЕМ. Уведомление центра (`im.notify.system.add`) кроме самого сообщения принимает `MESSAGE_OUT`
 * — «текст уведомления для внешних каналов, например, почты» (документация Битрикс24). Внутри
 * портала BB-коды поддерживаются и там, и в чате; а вот в письме, ушедшем по этому уведомлению,
 * разбирать их некому — человек получил бы буквальное `[URL=https://…]открыть приложение[/URL]`.
 * Пока `MESSAGE_OUT` не задан, портал берёт для внешнего канала сам `MESSAGE`.
 *
 * ⚠ Снимаем ТОЛЬКО настоящие квадратные скобки, то есть НАШУ разметку. Внешний текст (имя файла,
 * причина, имя поставщика) к этому моменту уже прошёл `neutralizeBb` и несёт полноширинные ［］ —
 * они остаются видимым текстом, как и задумано. Иначе обезвреженная ссылка из чужого документа
 * тихо восстановилась бы в рабочую разметку ровно в том канале, где её никто не проверяет.
 *
 * ⚠ Адрес из `[URL=…]` не выбрасывается, а печатается рядом с подписью: письмо без ссылки
 * бесполезно, а «открыть приложение» без адреса — обещание без выхода.
 *
 * ⚠ ВХОД ОБЯЗАН БЫТЬ УЖЕ ОБЕЗВРЕЖЕН (`chatSafeText`/`neutralizeBb`). Функция НЕ санитайзер: на сырой
 * чужой строке она не просто пропустит угрозу, а СОБЕРЁТ её — `[URL=https://evil]Оплатите здесь[/URL]`
 * превратится в «Оплатите здесь: https://evil», то есть в опрятный текст, который почтовые и пуш-клиенты
 * автолинкуют сами. Это ровно тот сценарий, ради которого в проекте живёт `neutralizeBb`, только на шаг
 * убедительнее. Держится не обещанием, а гардом: `tests/chatNotify.test.ts` требует, чтобы вызывающий
 * вне тестов был РОВНО ОДИН — тот, что получает уже собранное сообщение.
 */
export function bbToPlainText(message: string): string {
  return String(message ?? '')
    // [URL=адрес]подпись[/URL] → «подпись: адрес». Ссылок в наших сообщениях одна-две, и обе несут
    // подпись; форма «подпись: адрес» читается и в письме, и в пуше.
    .replace(/\[URL=([^\]]+)\]([\s\S]*?)\[\/URL\]/gi, (_m, url: string, label: string) => {
      const text = label.trim()
      return text ? `${text}: ${url}` : url
    })
    // ⚠ Незакрытый `[URL=адрес]` — отдельной строкой, ДО общей чистки тегов. Без неё общее правило
    // съедало такой тег как одиночный, вместе с адресом: обещание абзацем выше («адрес не
    // выбрасывается») переставало выполняться ровно там, где разметка пришла битой. Текущий
    // вызывающий шлёт только парные теги, поэтому вживую это не воспроизводилось, — но функция
    // экспортируется, и её документация не должна врать следующему.
    .replace(/\[URL=([^\]]+)\]/gi, '$1')
    // Остальные парные и одиночные теги — вон вместе со скобками, содержимое остаётся.
    .replace(/\[\/?[A-Za-z][^\]]*\]/g, '')
}
