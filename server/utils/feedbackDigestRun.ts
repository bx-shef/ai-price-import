 ...
 *
 * ⚠ Число выбрано, а не взято с потолка: тик часовой, значит три попытки укладываются в три часа —
 * этого хватает, чтобы пережить 429 и короткую недоступность Телеграма, и мало, чтобы отказ
 * навсегда (бота выкинули из чата) превратился в еженедельный поток обращений к приёмнику. Больше
 * трёх не помогает: то, что не доставилось за три часа, не доставится и за двадцать.
 */
export const MAX_ATTEMPTS_PER_WEEK = 3

/**
 * Сколько живёт счётчик попыток недели.
 *
 * Неделя плюс сутки запаса: ключ обязан пережить свою неделю целиком (иначе истечение посреди
 * недели открыло бы бюджет заново), но не задерживаться дольше следующей.
 */
export const DIGEST_COUNTER_TTL_SEC = 8 * 24 * 3600
...
export function createDigestRunner(deps: DigestRunDeps) {
  ...
  let localAttempts = 0
  /**
   * Наложение тиков внутри ОДНОГО процесса.
   *
   * ⚠ Границу называем явно (как `healthRunning` в проверке очередей): между экземплярами это не
   * защищает ничего — там работает общий счётчик недели. Здесь закрывается только случай, когда
   * обход приёмника пережил часовой тик.
   */
  let running = false
  ...
  return async function runDigest(): Promise<DigestRunResult> {
    ...
    try {
      ...
      if (delivered) {
        ...
        // безобиднее молчания.
        // Бюджет «дожимается» вызовами счётчика, а не установкой значения, потому что общий
        // счётчик умеет ровно одно — увеличивать с истечением (`WindowCounterStore`). Обрыв
        // посередине цикла закрывает неделю не до конца — то есть в худшем случае лишний повтор,
        // а не молчание; ради этого расширять общий интерфейс не стоит.
