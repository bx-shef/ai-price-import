// Manual-span helpers over @opentelemetry/api (телеметрия). The API is a thin, always-safe
// import: when NO SDK is registered (telemetry off — the default), `getTracer().startSpan()`
// returns a non-recording no-op span, so every helper here is ZERO overhead and changes
// nothing. The preload (otel.instrument.mjs) registers the real SDK only when an OTLP endpoint
// is set.
//
// All attributes go through the PII allowlist (`pickSafeAttributes`) — a caller physically
// cannot attach supplier / article / price / document content to a span (docs/PROJECT_MAP.md).
...
const TRACER_NAME = 'ai-price-import'
...
export interface DependencySpanInfo {
  /** External system: 'bitrix24' (extensible if more dependencies are timed). */
  system: string
 ...
 * `message` (which could carry supplier / article / amounts). Keeps only `[A-Za-z0-9_.-]`, caps
...
/**
 * Run `fn` inside a dependency span (B24 REST). On success tags `dep.status=ok`; on throw tags
 * `dep.status=error` + a sanitized `dep.error_kind`, sets ERROR status, and RETHROWS (never
 * swallows). No-op overhead when telemetry is off.
 */
