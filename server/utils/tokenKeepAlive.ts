// Proactive OAuth keep-alive (#175, port from client-bank). Bitrix `refresh_token` lives
// ~180 days; the lazy refresh (ensureFreshToken) only fires on a REST call, so a portal
// that is installed but idle (no documents imported, no polling) makes NO calls and its
...
// within a few days of expiry (age of the last saved token pair > ~177 days). We key off
// `portal_tokens.updated_at`, which saveToken/updateTokensOnRefresh stamp on every install
// AND every refresh, so it is exactly "when we last got a fresh pair".
//
...
// The refresh reuses ensureFreshToken (per-portal advisory lock + re-read + UPDATE-only
// persist, #35), so this path and the lazy path can't race on the rotating token.
...
/** The `updated_at` cutoff: a token last saved before this is near refresh-expiry. Pure. */
...
/**
 * Select the portals in the ~3-day near-expiry BAND — `updated_at` older than the cutoff
 * (near expiry) BUT not older than the full TTL (still refreshable) — oldest first, capped.
 * The lower bound matters: a grant that dies WITHOUT an uninstall (revoked / already past
 * 180d) keeps a frozen, ever-older `updated_at` and would otherwise monopolise the capped
 * batch forever, starving live near-expiry portals. Bounding to [now-TTL, now-cutoff) drops
 * such a row after ~`thresholdDays` runs and skips definitively-expired grants.
 */
...
export interface KeepAliveDeps {
  ...
  /** Force-refresh + persist a portal under its per-portal lock (reuses ensureFreshToken).
   *  Returns 'refreshed' on a successful rotation, 'skipped' if the portal vanished
   *  (uninstalled — nothing to keep alive). Throws on a dead grant (caller isolates). */
  refreshPortal: (memberId: string) => Promise<'refreshed' | 'skipped'>
  log?: (msg: string) => void
...
export interface KeepAliveSummary {
  selected: number
  refreshed: number
  /** Vanished before refresh (uninstalled). */
  skipped: number
  /** Refresh rejected (dead/expired grant, app removed) — logged, not fatal. */
  failed: number
}

/** member_id is a B24-issued hex id; clamp + strip control chars before logging. */
 ...
 * Only a failure of the initial selection propagates.
 */
export async function runTokenKeepAlive(deps: KeepAliveDeps): Promise<KeepAliveSummary> {
  ...
  for (const memberId of ids) {
    try {
      const outcome = await deps.refreshPortal(memberId)
      if (outcome === 'refreshed') s.refreshed++
      else s.skipped++
    } catch (e) {
      // Dead refresh_token (invalid_grant), removed app, etc. Log loud and carry on — the
      // portal needs a reinstall (user action); one bad portal must not block the others.
      s.failed++
