...
// GET /robots.txt — crawl policy for the public landing. Body is the pure `buildRobotsTxt`; the only
// I/O here is resolving the absolute base (configured deployment URL → canonical landing home).
// Deliberately NOT a static public/ file: the Sitemap: line needs an absolute host, which varies.

export default defineEventHandler((event) => {
  const base = siteBaseUrl(useRuntimeConfig(event).public.siteUrl as string)
