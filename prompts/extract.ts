...
//
// ⚠⚠ ЧИСЛА НИЖЕ НЕДЕЙСТВИТЕЛЬНЫ: правило 5 (артикул) переписано 2026-08-05, а любая правка текста
// их обнуляет — таково правило этого файла. Перезамер на корпусе владельца ИДЁТ; до его окончания
// цитировать цифры ниже нельзя. Что уже установлено отдельным парным прогоном на 31 реальном
// документе (472 позиции): прежняя редакция складывала в `article` ШТРИХКОДЫ EAN-13 — в одном
// счёте 35 из 42 «артикулов» оказались штрихкодами (префикс 481, Беларусь), причём у первой
// позиции настоящий артикул `B-WS3595` напечатан рядом и был проигнорирован. Новая редакция их не
// берёт: 263 артикула против ~250 верных у прежней, и ноль штрихкодов против 35.
// ⚠ Сводный показатель «доля позиций с артикулом» сам по себе ОБМАНЧИВ: он считал штрихкоды
// успехом, и по нему новая редакция выглядела хуже (285 → 263). Считать надо то, что попало в
// поле, а не сколько полей заполнено.
//
