...
Цикл такую работу прекрасно **находит**, но не делает. Если аналитика на проекте нет, она достаётся разработчику — и дальше два исхода, оба плохие. Либо он тратит на неё время, которого не закладывал в оценку, либо закрывает такие issue как «работает по ТЗ» и теряет ровно тот сигнал, ради которого всё затевалось.

У нас это был случай с российскими рублями: двадцать замечаний из двадцати шести и ни строчки кода в ответ — только правка требований. Если бы рядом был аналитик, он снял бы это за один разговор с отделом ещё до разработки.
