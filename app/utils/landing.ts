...
export const LANDING_INTEGRATORS
  = 'Внедряете Bitrix24 клиентам? Подключим AI-импорт прайсов под их процессы — свои поля, сущности и источники. Работу в вашем контуре обсуждаем индивидуально.'
...
export const LANDING_SOURCES = {
  ...
  text: 'Не хотите загружать файлы вручную? Забираем прайсы автоматически — из почты, Telegram, по FTP или из другого источника. Это индивидуальная доработка, по запросу — в том числе на вашем сервере.',
  /**
   ...
   */
  note: 'Текст документа при этом всё равно уходит на разбор к поставщику языковой модели: свой сервер сам по себе не делает контур замкнутым. Полностью локальный разбор — отдельный разговор.'
} as const
