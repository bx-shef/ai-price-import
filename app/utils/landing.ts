...
// ⚠ Импорты ОТНОСИТЕЛЬНЫЕ, не по алиасу `~/`. Этот модуль читает генератор карточки шаринга
// (`scripts/lib/ogTemplate.mjs` → `pnpm og`), а он гоняет исходник через `node
// --experimental-strip-types`, где алиасов Nuxt не существует. С алиасом команда падает
// `Cannot find package '~'` — и падала УЖЕ ДО #412: гард `ogStamp` требовал `pnpm og`, а `pnpm og`
// не запускался вовсе, то есть единственный документированный способ починить красный CI не
// работал. Тот же капкан, что с реестром редакций (`app/utils/legalArchive.ts`).
// Расширение `.ts` обязательно: node-загрузчик ESM его не достраивает.
import { PUBLISHER } from '../config/publisher.ts'
import { APP_NAME } from '../config/appIdentity.ts'

/**
 * Заголовок лендинга = ИМЯ ПРОДУКТА, без «в Bitrix24» (#412).
 *
 * ⚠ Суффикс убран по требованию юридического пакета: чужой товарный знак стоял внутри
 * собственного названия. Принадлежность к платформе никуда не делась — она в подзаголовке ниже,
 * в описании и в текстах карточки Маркета, где вопросов не вызывает.
 *
 * ⚠ Это не только `<title>`: та же строка идёт в OG/Twitter, в заголовок страницы установки и в
 * картинку карточки (`pnpm og`). Поэтому имя одно и берётся из общего источника.
 */
export const LANDING_TITLE = APP_NAME

/**
 * Заголовок ВКЛАДКИ и карточки шаринга: имя + отделённый тире дескриптор платформы.
 *
 * ⚠ Отдельная строка, а не имя с приписанным хвостом, и не наоборот. Прежний заголовок звучал
 * «AI-импорт прайсов в Bitrix24», то есть чужой знак стоял ВНУТРИ названия; убрав его целиком, мы
 * одновременно вынесли бы из `<title>` и из `<h1>` сильнейший поисковый ключ — «Bitrix24» набирают
 * вместе с «импорт», и страница перестала бы отвечать на такой запрос. Тире делает хвост
 * дескриптором: имя остаётся именем, платформа названа как площадка.
 */
export const LANDING_SEO_TITLE = `${APP_NAME} — приложение для Bitrix24`
export const LANDING_SUBTITLE
  = 'Накладные, счета, КП и прайсы разбираются автоматически прямо в Bitrix24: товары и контрагент подбираются по вашей базе, а на выходе — готовый лид, сделка, счёт или смарт-процесс с позициями, суммами и НДС 1-в-1.'
...
// ⚠ Цену тут не называем (#387). Приложение распространяется по подписке в Маркете, а обещание
// цены давалось БЕЗ УСТАНОВКИ — то есть до того, как человек увидит счёт. Тот же класс, что #341:
// обещание до, отказ после. Номинал тарифа ещё не выбран, поэтому формулировка верна при любом.
export const LANDING_HERO_NOTE = 'Приложение ставится из Маркета Bitrix24 и закрывает импорт. Настройку под ваш процесс берём на себя.'
...
// ⚠ «Установку в ваш контур … берём на себя» — то же утвердительное обещание, что снимали рядом,
// только сильнее (не «сделаем», а «принимаем ответственность»), и стоит оно ВЫШЕ оговорки (#96).
export const LANDING_WHY_SUBTITLE = 'Приложение из Маркета закрывает импорт. Настройку под ваши процессы берём на себя; работу в вашем контуре готовы обсудить отдельно.'

/** Промо-карточка «Приложение в Маркете» (точка входа, self-install). */
export const LANDING_MARKET_PROMO = {
  ...
  text: 'Приложение есть в Маркете Bitrix24. Установите за минуту и загружайте документы прямо в портале — тариф и условия смотрите в карточке Маркета.',
  cta: 'Открыть в Маркете Bitrix24'
} as const

/** Текст блока для интеграторов/партнёров. */
export const LANDING_INTEGRATORS
  = 'Внедряете Bitrix24 клиентам? Подключим AI-импорт прайсов под их процессы — свои поля, сущности и источники. Работу в вашем контуре готовы обсудить отдельно.'
...
export const LANDING_SOURCES = {
  ...
  text: 'Не хотите загружать файлы вручную? Забираем прайсы автоматически — из почты, Telegram, по FTP или из другого источника. Это индивидуальная доработка; работа на вашем сервере — по запросу.',
  /**
   * ⚠ Оговорка обязана стоять ТУТ, а не только в портальном виджете (#96).
   *
   * Она там уже была — но лендинг читают ДО установки, то есть до любого разговора с нами, и
   * человек уходил с готовым выводом «свой сервер = данные не покидают контур». Свой сервер этого
   * не даёт: разбор документа делает языковая модель поставщика.
   */
  note: 'Важно: текст документа при этом всё равно уходит на разбор к поставщику языковой модели — свой сервер сам по себе не делает контур замкнутым. Полностью локальный разбор — отдельный разговор.'
} as const
