...
/**
 * Канонический адрес страницы редакции.
 *
 * У ДЕЙСТВУЮЩЕЙ редакции канонический адрес — сам документ (`/eula`): тела байт в байт равны, и две
 * само-канонические страницы с одинаковым текстом дают дубль ровно на тех адресах, которые
 * открывает модератор Маркета. У ЗАМЕНЁННОЙ адрес свой — её текста больше нигде нет.
 *
 * Вынесено из вычисляемого свойства компонента намеренно: мутация «всегда `path`» переживала весь
 * набор тестов, потому что про канонический адрес страницы редакции не спрашивал никто.
 */
export function editionCanonicalPath(slug: string, date: string, e: LegalEditionEntry): string {
  return isCurrentEdition(e) ? `/${slug}` : editionPath(slug, date)
}

/**
 * Заголовок страницы редакции — обязан отличаться от заголовка документа: одинаковые `<title>` на
 * двух индексируемых адресах это дубль в выдаче и путаница в закладках. Вынесено по той же причине,
 * что и адрес выше: мутация «всегда заголовок документа» ничего не роняла.
 */
export function editionPageTitle(title: string, e?: LegalEditionEntry): string {
  return e ? `${title} — редакция от ${e.date}` : title
}
