/**
 * Pure builder for the `event.bind` / `event.unbind` batch the install page runs so
 * the backend receives Bitrix24 server events (ONAPPINSTALL / ONAPPUNINSTALL).
 *
 ...
 * batch call) is the caller's (install.vue).
 */
