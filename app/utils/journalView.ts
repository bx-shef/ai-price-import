...
/** Плитка даты слева от строки — как у дела в таймлайне Битрикс24. */
...
/**
 * Разбор даты на части для плитки-календаря.
 *
 * ⚠ Разбирается СТРОКОЙ, а не через `new Date`, по той же причине, что и `formatJournalDate`:
 * портал отдаёт своё локальное время со смещением, и `Date` пересчитал бы его в пояс браузера —
 * сотрудник в другом городе увидел бы у своего импорта не то время, что стоит рядом в карточке CRM.
 * ⚠ Номер месяца берётся из строки и клампится: `13` дал бы `undefined` в подписи, то есть
...
export function journalDateTile(iso: string): JournalDateTile | null {
  ...
  return { day: String(Number(d)), month, time: `${h}:${min}` }
}
