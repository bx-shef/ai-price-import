...
// Pure, no I/O. docs/PROCESS.md §6 «Запись в CRM».
...
function num(v: unknown): number | undefined {
  ...
  if (commas && dots) {
    ...
    // ОДНА запятая. По ru/be/kk-конвенции это десятичный разделитель — но модель нередко
    // перерисовывает «10 000» в английскую группировку «10,000», и тогда десятичное чтение даёт 10
    // вместо 10 000: тысячекратная ошибка в количестве (живой случай #302). Различаем по форме:
    // ровно три цифры после запятой И целая часть без ведущего нуля И дробной части больше нигде нет
    // — это группировка тысяч. «0,860» остаётся ценой (ведущий ноль), «1,5» — тоже (не три цифры).
    s = /^[1-9]\d{0,2},\d{3}$/.test(s) ? s.replace(',', '') : s.replace(',', '.')
  } else if (dots > 1) {
    s = s.replace(/\./g, '') // 1.234.567 → thousands grouping
