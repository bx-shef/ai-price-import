 ...
 *
 * Админ поменял настройки, и событие разошлось всем сотрудникам. Решение владельца 09.08.2026: **память СБРАСЫВАЕМ**, ручной выбор сотрудника не переживает правку.
 * Смысл рассылки в том, чтобы стало как настроил админ; память, пережившая её, ровно это и отменяла
 * бы — причём молча и у каждого по-своему.
 *
 * ⚠ Область — ОТКРЫТЫЕ экраны импорта. У сотрудника, чей экран в ту секунду закрыт, память в
 * sessionStorage переживает правку и восстановится при следующем открытии. С пусковой страницей
 * (#262) рабочий экран — это слайдер, который закрывают, так что случай не экзотический. Писать
 * «у всех» нельзя.
 *
