...
watch([CHOICES, () => etid.value], () => {
  ...
  failToAuto('entity')
// ⚠ `immediate: true` — НЕ украшение. `useCrmTypes` это singleton с модульным кэшем: список типов
// грузится один раз на сессию. Значит у ВТОРОГО и последующих пикеров (слайдер настроек, второе
// открытие `/app`, каждое правило маршрутизации на экране настроек) `CHOICES` вычисляется сразу с
// готовым списком и больше НЕ МЕНЯЕТСЯ — а ленивый `watch` срабатывает только на изменение. То есть
// исчезнувший смарт-процесс ловился ровно один раз за сессию, у самого первого пикера, и это
// выглядело работающим механизмом: на первом открытии портала он честно срабатывал.
}, { immediate: true })
/**
