...
<template>
  ...
  <li class="flex gap-3 rounded-lg border border-(--ui-color-base-5) bg-(--ui-color-base-7) p-3">
    ...
    <div class="flex min-w-0 flex-1 flex-col gap-2.5">
      ...
      <!-- ⚠ `pick-file` у УПАВШЕЙ загрузки (#506 п.3): импорт, не дошедший до CRM, не оставил ни
           записи, ни дела с вложением — документ брать неоткуда, и человек выбирает его сам. У
           успешной загрузки дело есть, и файл читает сервер (#461) — там выбирать нечего.
           ⚠ Признак берётся по СТАТУСУ строки, а не по наличию дела: наличия дела страница не
           знает, а спрашивать портал ради подписи в форме — лишний запрос на каждую строку. -->
      <FeedbackWidget
        v-if="meta.terminal && job.status !== 'expired'"
        :job-id="job.jobId"
        :file-name="job.fileName"
        :pick-file="job.status === 'error'"
      />
    </div>
  </li>
</template>
