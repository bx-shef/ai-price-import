...
// so it inherits size/colour), paired with a visually-hidden but selectable «BYN» so
...
<template>
  ...
  >
    <svg
      ...
      aria-hidden="true"
    >
      <!-- Official National Bank of Belarus BYN sign. Traced glyph, source:
           https://commons.wikimedia.org/wiki/File:BYN_symbol.svg (public-domain state symbol). -->
    ...
    </svg><span class="byn-br">BYN</span>
  </span>
...
/* Visually hidden but present for screen readers and copy/paste (the SVG carries the
   visual, this carries the text «BYN»). */
.byn-br {
  position: absolute;
  width: 1px;
  height: 1px;
