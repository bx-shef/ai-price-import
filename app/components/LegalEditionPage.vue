...
//
// ⚠ `fatal: true` обязателен, и это найдено живой проверкой в браузере. Ниже стоит `await`, а он
// делает весь `setup` асинхронным — брошенная ошибка тогда отвергает границу Suspense, а не
// уходит в обработчик ошибок Nuxt. При переходе ВНУТРИ приложения (со страницы архива по битой
// ссылке) это давало пустую белую страницу с прежним заголовком и без кода 404: адрес из договора
// открывался «ничем». Полная загрузка страницы при этом отдавала честный 404 — то есть дефект был
// виден только кликом, не запросом.
if (!edition.value || !doc.value) {
  throw createError({ statusCode: 404, statusMessage: 'Редакция не найдена', fatal: true })
}
...
if (!source) throw createError({ statusCode: 404, statusMessage: 'Текст редакции недоступен', fatal: true })
</script>
