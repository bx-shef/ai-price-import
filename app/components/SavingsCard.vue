 ...
 *
 * ⚠ Условие `mobile` здесь ТАКОЕ ЖЕ, как у первой загрузки, и это не дублирование: скрыт условным
 * рендером только ШАБЛОН, а сам компонент на телефоне смонтирован — значит `ref` со страницы на
 * него указывает и вызов доходит. Без проверки после каждой пачки уходил бы запрос за метриками
 * ради карточки, которой на экране нет; а комментарий выше обещал ровно обратное — «нет карточки,
 * нет и запроса». Обещание в комментарии, которого код не выполняет, хуже отсутствия комментария.
 */
defineExpose({ reload: (opts?: { silent?: boolean }) => props.mobile ? undefined : load(opts) })
</script>
