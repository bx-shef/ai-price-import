...
<template>
  <!-- Уведомление о cookie (#404) живёт в КОРНЕ, а не в layout лендинга: юридические страницы
       ...
       ⚠ ПЕРЕД содержимым: полоса нарисована снизу, но в порядке обхода с клавиатуры стояла
       последней — до её кнопок было 44 нажатия Tab на лендинге, то есть блокирующий вопрос
       доставался клавиатурному посетителю после всей страницы. Визуально позиция в разметке ничего
       не меняет: элемент `fixed`. -->
  <CookieBanner />
  <NuxtLayout>
    <NuxtPage />
  </NuxtLayout>
