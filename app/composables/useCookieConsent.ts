...
export function useCookieConsent() {
  ...
  /**
   * Забыть решение — вопрос будет задан снова.
   *
   * ⚠ Нужен потому, что иначе передумать средствами сайта нельзя вовсе: баннер виден, только пока
   * ответа нет. «Дать согласие в один клик, отозвать — очисткой данных браузера» — самое слабое
   * место такой конструкции, и Политика сайта прямо обещает возможность изменить решение.
   * ⚠ Уже поднятый счётчик обратно не выключается: скрипт загружен, и честно об этом сказано в
   * документе — отказ вступает в силу со следующей загрузки страницы.
   */
  const reset = () => {
    choice.value = null
    try {
      window.localStorage.removeItem(CONSENT_KEY)
    } catch { /* нечего чистить — значит и решения нет */ }
  }

  return { choice, ready, load, decide, reset }
}
