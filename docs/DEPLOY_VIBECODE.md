...
- `GET /app` · `/import` · `/settings` · `/metrics` · `/login` · `/install` → 200 (пререндеренные страницы);
...
3. **Rate-limit `/api/auth/login`** (антибрутфорс пароля оператора) — был `limit_req` по IP в nginx; в
   приложении остаётся только 400 мс задержки на неудачную попытку, **IP-троттла нет**.
...
> `PATH` (`AGENT_BIN=claude`), иначе пайплайн падает «spawn claude ENOENT».
...
> агент не запускается. `NUXT_PUBLIC_SITE_URL` подставь **после первого деплоя** (когда узнаешь `appUrl`) и
> передеплой — из него строится абсолютный URL хендлера событий `/api/b24/events` (иначе `/install`
> откажется биндить). `B24_APPLICATION_TOKEN` — **пустой**: он приходит в `ONAPPINSTALL` и пишется в **БД**
...
`appUrl/api/health` → `ok`. Сделай снимок диска (`POST /:id/backups`).
