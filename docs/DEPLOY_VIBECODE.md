...
2. **Пер-IP троттлы — ✅ по реальному peer.** Без nginx `X-Forwarded-For` подделываем клиентом, поэтому под
   ...
   `socket.remoteAddress` = единый шлюз → все клиенты в одном ведре (демо/логин лочатся для всех). Худший
   случай безопасно (глобальный кап = нет cost-drain/брутфорса, только просадка доступности), но чинится
   без передеплоя кода: `curl` заголовки/поведение на живом тоннеле; если тоннель — **доверенный** прокси,
   добавляющий реальный IP последним хопом XFF, поставь **`APP_EDGE_TRUST_XFF=1`** (тогда ключ = последний
