...
| Заголовок вкладки и `lang` | Открыть `/` | `<html lang="ru">`; `<title>` = «AI-импорт прайсов в Bitrix24»; `bodyAttrs.class` с `bg-[#05010f]` | [вручную] |
...
| Извлечение: много-листовая книга (GH #76) | Загрузить ТТН/накладную с товарами на не-первом листе («Приложение») | Боевой `officeToText` И демо-путь (`xlsxToText`/`xlsxToTextFallback`) читают **все листы**, склейка, товары не теряются; пустые отброшены, число листов ограничено (демо↔прод согласованы) | [нужен бинарями/авто] |
...
| Жизненный цикл: ошибка/оверсайз | Битый/пустой/огромный файл | `status='error'`, русское сообщение; на `/app` красным; текст >500000 → «разбейте на части» без обрезки | [нужен живой портал] |
| UI `/app` вне портала | Загрузить страницу | Рендерится, пайплайн не идёт → error в оранжевом блоке | [вручную] |
| UI `/app` (загрузка) в портале | drag-drop / `<input multiple>` в зоне вверху; «Обновить» | Последовательный upload, `input.value=''`; список перечитывается, статус-бейджи | [нужен живой портал] |
| `/import` → редирект | Открыть `/app`… нет, `/import` | Мгновенный редирект на `/app` (JS `router.replace` + `http-equiv=refresh` no-JS) | [вручную] |
...
| Приоритет ТП/SKU | `tests/offerLookup.test.ts`, `tests/productLookup.test.ts` | ТП ищутся первыми (xmlId→имя, `active:'Y'`), попадание → `offer.id`; fail-soft фолбэк на товар без каталога; XML_ID+ACTIVE-подбор | [авто] + ✅ live |
| Сверка цены/НДС/итога | `tests/pricing.test.ts`, `tests/crmSyncCore.test.ts` | Построчный НДС; сверка печатного итога (net/incl/оба/ни один); асимметрия (не флипать в «с НДС»); три-стейт флага→`corrected`; **скидка** (−цена)→итог; mixed-VAT; tolerance масштабируется; `totalMismatch`→warning; частичная запись (skip-warn)→warning «не включает пропущенные» | [авто] + ✅ live |
| Единицы (русские, не-deprecated) | `tests/measureList.test.ts` | `catalog.measure.list` + карта `SYSTEM_MEASURE_RU` (системные меры null→«шт»/«кг»); кастомные — из API; `fetchMeasureRows` обогащён для индекса | [авто] + ✅ live |
| Механика импорта (staging, 1-за-1) | `tests/nuxt/importStaging.nuxt.test.ts`, `importJobItem.nuxt.test.ts` | Файлы стейджатся (не авто-загрузка); импорт по кнопке 1-за-1; стабильный UUID-ключ; дедуп/кап/ретрай; уведомление; имя файла→ссылка на Диск (клик открывает Disk-путь, не сущность) | [авто] |
...
**[нужен живой портал] Bitrix24 (env `B24_HOOK`, scopes `crm,catalog,disk,im`):**
