...
| Endpoint happy-path | `curl -F file=@public/demo/kp-ru.txt localhost:3000/api/demo/extract` | 200 `{result, notice, remaining}`; `remaining` убывает | [вручную] |
...
| Неверное расширение | `curl -F file=@some.pdf` | 415 с текстом про PDF/скан/Excel | [вручную] |
...
| Реальное извлечение PDF/скан/Excel через `runAgent` | — | В демо НЕ реализовано (детерминированное ядро); проверять в полной версии | [нужен LLM-ключ] |
| Запись в Bitrix24 | — | В демо-пути отсутствует полностью | [нужен живой портал] |

### Публичный лендинг

Файлы: `app/pages/index.vue`, `app/components/HeroGraph.vue`, `app/utils/landing.ts`, `app/app.vue` (SEO). Тест: `tests/landing.test.ts`.
...
| HeroGraph: reduced-motion | Включить «уменьшение движения», перезагрузить `/` | Статичный кадр, `requestAnimationFrame` не стартует | [вручную] |
| HeroGraph: пауза вне видимости/скрытой вкладки | Проскроллить hero, переключить вкладку | Импульсы замирают, при возврате продолжаются (~30fps) | [вручную] |
| HeroGraph: очистка ресурсов | Переходы `/`↔`/app` несколько раз | Нет накопления слушателей/RAF; `running=false` после ухода | [вручную] |
| HeroGraph client-only / SSR | `pnpm generate` + просмотр без JS | Генерация не падает на canvas/`window`; canvas `aria-hidden`, слой `pointer-events-none` | [вручную/авто] |
...
| HeroGraph не покрыт тестами | Ручная визуальная проверка через `pnpm dev` | Скрипта скриншотов нет — снимать вручную | [вручную] |
...
| Извлечение: office | Загрузить `.xlsx`/`.docx`/`.xls` | `officeToText` → txt, временный каталог удаляется | [нужен бинарями] |
