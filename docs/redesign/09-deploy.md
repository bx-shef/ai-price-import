...
приложение обслуживает всех арендаторов на скромном сервере, документы обрабатываются **по очереди**,
а не пачками параллельно. Кому нужны скорость/объём — предлагаем разворачивание на своём сервере
(см. маркетинг §2, блок про self-hosted).

Что реально ест ресурсы — **внешние процессы на один документ**, не Node:

| Этап | Бинарь | CPU | RAM (пик) |
|------|--------|-----|-----------|
...
**Минимум (обработка по одному документу):** **2 vCPU / 2 ГБ RAM**. Этого достаточно для стабильной
работы очереди — просто медленно, и это ОК по дизайну.

**Ключевая ловушка — конкуренция, а не объём.** Один `tesseract` сам разворачивает потоки на все
ядра (замер: 2.3с wall при ~7с CPU-времени). На 4 ядрах **три** документа параллельно (2×OCR +
libreoffice) устроили переподписку → каждый OCR перевалил за таймаут `RUN_TIMEOUT_MS` (90с) и
документ **ложно уехал в ошибку**, хотя автономно шёл 2 секунды. Поэтому на минимальной инфраструктуре:

1. **`OMP_THREAD_LIMIT=1`** (или `2`) в env бэкенда — чтобы N процессов tesseract не дрались за ядра.
2. **Конкуренция извлечения ≤ числу ядер** — не больше, иначе тяжёлый OCR превысит 90с-таймаут.
3. **`mem_limit` обязателен** (2–4 ГБ) — OOM-защита от бомб (таймаут RAM не ограничивает).
