...
**⚠️ Важное расхождение конфигурации с этой позицией (нужно исправить кодом).** Воркер сейчас
**хардкодит** конкуренцию: `server/queue/worker.ts` — `extract: concurrency 4`, `agent: 2`,
`crm: 4`. На минимуме 2 vCPU это **больше числа ядер** → та самая переподписка (см. ниже). Значения
нужно вынести в env и на минимальном профиле снизить (`extract`/`agent` = 1). Issue #95.
...
1. **`OMP_THREAD_LIMIT=1`** (или `2`) в env бэкенда — чтобы N процессов tesseract не дрались за ядра.
   ⚠️ Сейчас **не выставлено** (нет ни в `.env.example`, ни в compose) — это рекомендация/TODO, не факт.
2. **Конкуренция извлечения ≤ числу ядер** (`server/queue/worker.ts`, см. расхождение выше) — иначе
   тяжёлый OCR превысит 90с-таймаут.
