...
2. **Init/seed-контейнер** в нашем `docker-compose.prod.yml`, который на старте копирует файл в
   ...
   но не пересоздание тома прокси. Менее чисто, чем bind-mount.
...
общем — вариант 3 до согласования с владельцем стека currency-converter. Полная автоматизация
(вариант 1 для текущего хоста) остаётся в GH #71 как задача, требующая доступа к чужому compose.
