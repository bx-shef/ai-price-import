...
**неидемпотентное** создание (`crm.item.add`); любой in-SDK-ретрай —
...
упасть всей BullMQ-джобе и ретраим её — там создание идемпотентно (`findExisting`-перед-созданием по
маркеру #135 для сущности; создание товара удалено, `findProduct` — только поиск). **Остаётся включённым**
