...
> **Важно (решения Q4/Q7).** Новый продукт — **облачное приложение для Маркета Bitrix24**
> (мультитенант, OAuth), а не инструмент под один портал. И **весь код для ядра/коробки Б24
> убирается** — интеграция идёт только через MCP поверх **стандартного REST**. Поэтому:

- **Переносим ценность:** промпт (`prompts/main.md`), бизнес-логику инструментов (но **переписываем
  на стандартный `crm.*`/`catalog.*`** вместо кастомных `shef:purchase.api.*`), ядро извлечения текста,
  eval-харнесс, homoglyph-логику, а также **OAuth/мультитенант-код** старого MCP (он оживает).
...
- **Выкидываем:** весь `b24-controller/` (PHP-модуль `shef.purchase`, `procure*.php`, **патч ядра НДС** —
  ...
  хардкод инфоблоков/полей (заменяем пер-портальным маппингом в настройках); i18n на 19 локалей.
