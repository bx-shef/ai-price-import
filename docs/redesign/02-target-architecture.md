...
- **Отзывы сотрудников** — на результате импорта: 👍 / 👎 / 💡(идея) + опц. текст. Хранятся
  (Postgres, per-portal), питают метрики качества, видны оператору/владельцу. (В старом проекте —
  durable outbox → GitHub-issues; здесь канал издателя опционален/анонимен.)
...
- **Очередь ручного разбора** (кандидат) — кейсы «товар не найден / неоднозначно / ошибка» + просмотр
  отзывов. *Уточнить:* у издателя кросс-портально или у клиента in-portal (открытый вопрос Q13).
