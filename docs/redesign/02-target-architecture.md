...
5. **Пер-портальная настройка маппинга.** Приложение generic → нельзя хардкодить инфоблоки/поля
   ...
   **поле артикула поставщика** (+ его тип и разделитель, см. ниже), **целевую сущность и её
   направление** (см. ниже), правила НДС, **настройки дела** (текст/тип/ответственный/срок) и
   **тумблер прикрепления файла**, **стратегию поиска товара** (по названию целиком / по артикулу),
   **стратегию при отсутствии товара** (создать товар с артикулом / пропустить строку+предупреждение /
   произвольная позиция), **дефолт единицы измерения**, **чат ошибок** (`dialogId`) и чат уведомлений.
   Чистое ядро маппинга — в `app/utils` с тестами (как матрицы эталона). **Договор в маппинг не входит** (Q8) — платная доработка. **Налоговый ID
...
- **Исходный файл и дело (вместо коммента/UF-поля).** Файл кладём на **Диск Б24**: общий диск
  (`disk.storage.getlist`) → своя папка приложения → подпапка **по месяцам** (`YYYY-MM`,
  `disk.folder.addsubfolder` идемпотентно) → `disk.folder.uploadfile`. Затем на целевой сущности
  создаём **«дело»** (`crm.activity.todo.add`, универсальное — как в эталоне) с прикреплённым файлом.
  **Настраивается:** текст/тип/ответственный/срок дела — из настроек портала; **само прикрепление
  файла — тумблер** (вкл/выкл). **Комментарий в таймлайн и UF-поле в сущности НЕ пишем** (уходим от
  модели старого проекта `UF_CRM_DEAL_SH_PRCHS_AI_FILE` + `COMMENTS`).
