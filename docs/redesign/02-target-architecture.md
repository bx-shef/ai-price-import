...
5. **Пер-портальная настройка маппинга.** Приложение generic → нельзя хардкодить инфоблоки/поля
   ...
   **поле артикула поставщика** (+ его тип и разделитель, см. ниже), **целевую сущность и её
   направление** (см. ниже), правила НДС (см. Q12), **тумблер «сохранять файл или нет»** (само дело —
   ...
   пропустить строку+предупреждение / произвольная позиция), **настройки единиц измерения** (набор — см. Q11),
   **чат ошибок** (`dialogId`) и чат уведомлений. Чистое ядро маппинга — в `app/utils` с тестами (как матрицы эталона). **Договор в маппинг не входит** (Q8) — платная доработка. **Налоговый ID
...
- Целевая сущность — универсальный `crm.item.add` по `entityTypeId` (2=сделка / ≥1000=смарт-процесс /
  ...
  `crm.item.productrow.set` (для КП — `crm.quote.productrows.set`), штатный НДС.
...
| **Единица измерения не найдена** | Дефолт/авто-создание **из настроек приложения**; при этом фиксируем как **ошибку → сообщение в чат ошибок.** |
