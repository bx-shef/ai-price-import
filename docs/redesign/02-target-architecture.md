...
5. **Пер-портальная настройка маппинга.** Приложение generic → нельзя хардкодить инфоблоки/поля
   ...
   **Кто решает цель:** (1) **вручную у файла при импорте** — селектор тип+направление рядом с
   загруженным файлом (override на файл); (2) **AI-агент сам** — по **правилам роутинга** портала.
   Приоритет: ручной override на файл → авто-роутинг по правилам → **дефолтная цель**.

   **Правила роутинга (в настройках), два режима:**
   - **Единый:** всё → одна цель (напр. «всё в сделки, направление ЧЧЧ»).
   - **По типу/ключевым словам:** правила вида «тип документа → цель+направление» и «в документе есть
     слово → цель+направление». Примеры: счёт→Инвойс, КП→КП; если в документе встречается «ТН»
     (товарная накладная) → сделка в направлении ХХ. Первое сработавшее правило → цель; иначе дефолт.
     Классификацию типа/поиск ключевых слов делает агент (по тексту документа, язык-агностично);
     сами правила — чистое ядро (`app/utils`, тесты).
