...
| **Валюта не определена** | **Ошибка → чат ошибок.** |
...
Тонкости (общая дисциплина, детали — при реализации): при **нескольких** совпадениях (поставщик по
`RQ_INN` не уникален; товар по артикулу/названию) — детерминированный выбор (напр. минимальный ID) +
предупреждение; некорректные цена/количество (не число/отриц.) — клампим/помечаем. Эти под-случаи —
кандидаты в открытые вопросы (см. ниже, спрашиваем владельца).
