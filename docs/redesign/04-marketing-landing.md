...
- **Позиционирование по скорости — «стабильно, а не быстро» (важно, вшить в тексты и калькулятор).**
  Облачное приложение работает на **общей минимальной инфраструктуре** и обрабатывает документы
  **по очереди** — это осознанный выбор (мы не вкладываемся в производительность общего сервера, см.
  [09-deploy §«Ресурсы воркера»](09-deploy.md)). Честный месседж клиенту: **распознавание надёжное, но
  не мгновенное**; в пик очередь может подождать. Не обещаем «за секунды» — обещаем «сам, точно, без
  ручного ввода».
  - **Апселл «нужно быстро / большие объёмы → развернём на вашем сервере» (self-hosted).** Выделенные
    ресурсы клиента = параллельная обработка без общей очереди. Модель разворачивания «под клиента» —
    как **«чёрная дыра» у вайбкодинг-Битрикс24** (self-hosted элемент разворачивания для **РФ**).
    ⚠️ Для **РБ** условия/юрисдикцию/хостинг нужно **уточнить** (TODO перед выносом в оффер) — не
    копировать РФ-модель вслепую. Идёт тем же каналом, что индивидуальная доработка
    (`CustomDevCard`/`BriefForm`).
  - **Калькулятор расчётов:** добавить тариф/опцию **«на вашем сервере» (self-hosted)** рядом с
    облачной подпиской — с явной развилкой «облако (по очереди, дешевле) ↔ свой сервер (быстро,
    выделенно)». Для РФ — по модели «чёрной дыры»; для РБ — плейсхолдер «по запросу», пока условия не
    уточнены. (Калькулятора как компонента пока нет — это требование к нему, а не готовая цена.)
