...
- **Блок «Индивидуальная доработка под ваш процесс»** — платная фича поверх стандартного приложения
  ...
  **по артикулу**, **загрузку вне портала** (standalone / приём по e-mail / **Telegram** / подхват из
  папки по **FTP/SFTP** / из другого источника — на сервере клиента) и другие правила под ваш учёт».
  Сюда уходят убранные из стандарта фичи — подбор договора (Q8) и внепортальная загрузка (стандартное
  приложение работает **только внутри портала Б24**, в т.ч. в **мобильном приложении**) — как продающее
  предложение, а не ядро продукта.
  - **Автосбор прайсов из внешнего источника — на лендинге** (`LANDING_SOURCES`, блок в секции
    «Обсудить индивидуальную интеграцию»): «Забираем прайсы автоматически — из почты, Telegram, по FTP
    или из другого источника. Это индивидуальная доработка **на вашем сервере**». Облачное приложение
    Маркета этого не делает (работает только внутри портала) — это платная интеграция под контур клиента.
