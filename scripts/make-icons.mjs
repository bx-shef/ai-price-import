...
// ⚠ Импорт ОТНОСИТЕЛЬНЫЙ и с расширением: генератор гоняется через `node`, где алиасов Nuxt нет.
import { APP_NAME } from '../app/config/appIdentity.ts'
...
try {
  ...
  await writeFile(join(PUB, 'site.webmanifest'), `${JSON.stringify({
    // ⚠ Имя — из общего источника (#412). Прежде здесь стоял литерал с чужим товарным знаком
    // внутри названия, и он же приезжал в диалог установки PWA. Генератор перезаписывает
    // `site.webmanifest`, поэтому правка одного файла без правки этого откатывалась ближайшим
    // `pnpm icons`.
    name: APP_NAME,
