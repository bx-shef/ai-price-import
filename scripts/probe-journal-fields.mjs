...
//
// ═══ ЧТО ПОКАЗАЛ ПРОГОН 10.08.2026 (портал b24-hrbvzq, вебхук) ═══════════════════════════════
//
//   1. `crm.activity.list` при ЯВНОМ перечислении полей отдаёт `DESCRIPTION` целиком, `FILES`
//      парой `id`+`url` и цвет — ВНУТРИ `SETTINGS` (`{"COLOR":"7"}`). Отдельного поля `COLOR`
//      у дела нет ни в списке, ни в `get`.
//   2. Значит батч по строкам НЕ НУЖЕН: страница журнала это по-прежнему ОДИН запрос к порталу.
//   3. `FILES` отсутствует, когда у дела нет вложения, — и это, судя по всему, ввело в заблуждение
//      прошлый прогон (`probe-activity-files`, 06.08.2026): там записано «list не отдаёт FILES
//      даже со `select:['*']`». Та запись помечена как пересмотренная.
//   4. Адрес файла приходит вида `.../bitrix/tools/crm_show_file.php?fileId=…&auth=` — с ПУСТЫМ
//      `auth`. Открывается ли он новой вкладкой на сессии портала — вживую НЕ проверено; это
//      пункт приёмки для владельца, а не вывод отсюда.
//
// ═════════════════════════════════════════════════════════════════════════════════════════════
//
