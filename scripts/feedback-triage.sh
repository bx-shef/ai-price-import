...
#   - ПРИВАТНОСТЬ: репо кода bx-shef/ai-price-import — ПУБЛИЧНЫЙ. НЕ копируйте сырой
#     контекст отзыва (jobId, имя файла, № сделки, УНП, названия компаний) в issue
#     ПУБЛИЧНОГО репо. Только обезличенная суть + ссылка на приватный отзыв. Перед
#     хранением контекста задайте ОТДЕЛЬНЫЙ приватный FEEDBACK_REPO и сверьте его
#     приватность вживую (GET /repos/{owner}/{repo} → private). См. §2, §5 доки.
set -euo pipefail

API="https://api.github.com"

# Целевые репозитории — параметризованы (переопределяемы через env).
# PROJECT_REPO — публичный репо кода/задач; FEEDBACK_REPO — приватный приёмник отзывов
# (тот же ENV GITHUB_FEEDBACK_REPO, что у ingestion-канала). Дефолт FEEDBACK_REPO
# указывает на код-репо ТОЛЬКО как fallback — он ПУБЛИЧНЫЙ, поэтому перед хранением
# клиентского контекста задайте отдельный приватный репо и сверьте приватность.
PROJECT_REPO="${PROJECT_REPO:-bx-shef/ai-price-import}"                       # публичный репо кода/задач
FEEDBACK_REPO="${FEEDBACK_REPO:-${GITHUB_FEEDBACK_REPO:-bx-shef/ai-price-import}}" # приватный репо отзывов (сверь!)
