...
//
//      ⚠⚠ ПЕРЕСМОТРЕНО 10.08.2026 (`probe-journal-fields.mjs`, #495): ЭТО НЕВЕРНО. И `get`, и
//      `list` ОТДАЮТ `FILES` (парой `id`+`url`), `DESCRIPTION` и цвет в `SETTINGS.COLOR` — но
//      только при ЯВНОМ перечислении полей в `select`. Разница была в форме запроса, а не в
//      недоступности полей; вдобавок у дела того прогона вложения, судя по всему, не было вовсе, и
//      отсутствие поля читалось как «портал не отдаёт». Запись оставлена вместе с опровержением:
//      вычеркнутая ошибка полезнее удалённой — она показывает, КАК ошиблись.
