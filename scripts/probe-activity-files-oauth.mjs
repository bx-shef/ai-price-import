...
//
// ═══ ОТВЕТ ПО КОДУ ЯДРА (06.08.2026) — вопрос закрыт, скрипт оставлен как воспроизведение ══════
//
//   ⚠ Прикрепить файл, УЖЕ лежащий на Диске, ПО ЕГО ID — по REST нельзя. `tryInternalizeDiskFileField`
//   пропускает элемент без ключа `fileData` (`continue`), поэтому все формы вида `{id}`/`[N]`/`"nN"`
//   принимаются и молча отбрасываются. Работают ровно две: `{fileData:[имя, base64]}` (грузит байты
//   = ВТОРАЯ копия на Диске) и `{id, remove:'Y'}` (открепить). `FILE_ID` — внутренний, рождается
//   только из загрузки байт; `STORAGE_ELEMENT_IDS`/`STORAGE_TYPE_ID` вырезаются в `innerUpdate`,
//   и полученный там `ERROR_CORE` — дефолтный код неподдерживаемой формы, а НЕ «живой путь,
//   вызванный неверно» (так стояло в п. 4 ниже — неверно). Ядро связывать по id умеет
//   (`CCrmActivity::DoSaveElementIDs` → `b_crm_act_elem`, без копии), но наружу метода нет, а мы
//   в облаке — своего PHP не поставить.
//
//   ⚠ Иконка нативного To-Do захардкожена (`ToDo::getIconCode()`), меняется только `colorId`
//   (`default`, 1…8). Своя иконка и теги (`TagDto`: `title` + `type` ∈ success/failure/warning/
//   primary/secondary) — только в `layout` КОНФИГУРИРУЕМОГО дела, а оно файлов не носит. То есть
//   «файлы + своя иконка» в одном деле недостижимо; у нативного To-Do пользовательских тегов нет
//   вовсе (`SETTINGS['TAGS']` = один системный `OVERLAP_EVENT`).
//
// ═════════════════════════════════════════════════════════════════════════════════════════════
//
