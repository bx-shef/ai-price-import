...
describe('колонка «Стоимость» — неоднозначная, роль решает печатный итог (GH #76)', () => {
  // ⚠ Прежде слово не разбиралось ВООБЩЕ, и таблица, где это единственная денежная колонка,
  // теряла все деньги: позиции показывались без цен, а «итого» брался только из подвала.
  // Отнести его наугад к цене или к сумме было бы хуже — числа встали бы не в ту колонку.
  const table = (money: string[], total: string) => [
    'Наименование;Кол-во;Стоимость',
    ...money,
    `Итого;;${total}`
  ].join('\n')

  it('Σ стоимость сходится с итогом ⇒ это ИТОГ ПО СТРОКЕ', () => {
    const r = extractDemo(table(['Кабель;2;200,00', 'Розетка;3;300,00'], '500,00'))
    expect(r.items.map(i => [i.price, i.sum])).toEqual([[undefined, 200], [undefined, 300]])
    expect(r.warnings.join(' ')).not.toContain('Стоимость')
  })

  it('Σ (кол-во × стоимость) сходится с итогом ⇒ это ЦЕНА ЗА ЕДИНИЦУ', () => {
    const r = extractDemo(table(['Кабель;2;200,00', 'Розетка;3;300,00'], '1300,00'))
    expect(r.items.map(i => [i.price, i.sum])).toEqual([[200, undefined], [300, undefined]])
    expect(r.warnings.join(' ')).not.toContain('Стоимость')
  })

  it('итога нет ⇒ кладём в сумму, но ГОВОРИМ, что это догадка', () => {
    const r = extractDemo(['Наименование;Кол-во;Стоимость', 'Кабель;2;200,00'].join('\n'))
    expect(r.items[0]?.sum).toBe(200)
    expect(r.warnings.some(w => w.includes('Стоимость'))).toBe(true)
  })

  it('есть и «Цена», и «Сумма» ⇒ «Стоимость» НЕ трогаем: это третья денежная колонка', () => {
    // ⚠ Раскладка «Цена | Стоимость | Сумма» реальна, и угадывание сместило бы разбор.
    const r = extractDemo([
      'Наименование;Кол-во;Цена;Стоимость;Сумма',
      'Кабель;2;100,00;999,00;200,00'
    ].join('\n'))
    expect(r.items[0]?.price).toBe(100)
    expect(r.items[0]?.sum).toBe(200)
  })

  it('временное поле разбора наружу не уходит', () => {
    // Иначе интерфейс показал бы третью денежную колонку без подписи.
    const r = extractDemo(table(['Кабель;2;200,00'], '200,00'))
    expect(Object.hasOwn(r.items[0] ?? {}, 'amount')).toBe(false)
  })
})
