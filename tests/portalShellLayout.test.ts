...
describe('оболочка in-portal экранов', () => {
  ...
  it('/app: две колонки включаются НЕ РАНЬШЕ lg', () => {
    ...
    expect(t).toMatch(/grid[^"]*lg:grid-cols-3/)
    expect(t, 'колонки не должны включаться на sm/md').not.toMatch(/(?:sm|md):grid-cols-[23]"/)
  })

  it('/app: порядок в разметке — как на телефоне, местами колонки меняет только lg:order-*', () => {
    // Программа чтения и Tab идут по РАЗМЕТКЕ. Если поменять местами сами блоки, а не их
    // `order`, то на телефоне человек получит журнал раньше экономии — то есть ровно ту раскладку,
    // которую владелец забраковал, и заметно это будет только с телефона.
    const t = template('app/pages/app.vue')
    expect(t).toMatch(/<aside[^>]*lg:order-2/)
    expect(t).toMatch(/lg:order-1[^>]*>[\s\S]{0,4000}?<ImportJournal/)
  })
