...
describe('оболочка in-portal экранов', () => {
  ...
  it('/app: порядок в разметке — как на телефоне, места колонок заданы отдельно', () => {
    ...
    expect(metrics, 'экономия идёт до журнала').toBeLessThan(journal)
    // ⚠ Баннер теперь ВНУТРИ ленты, врезкой после третьей записи (#507, решение владельца по живому
    // снимку с телефона). Прежняя редакция требовала обратного — «баннер выше журнала», — и этот
    // гард честно покраснел на правке. Порядок между двумя решениями рассудил телефон: сверху
    // баннер занимал целый экран перед тем, ради чего приложение открывают.
    expect(promo, 'баннер отдаётся журналу слотом, то есть стоит ПОСЛЕ него в разметке').toBeGreaterThan(journal)
  })
  ...
  it('/app: метрики стоят ВЫШЕ журнала, баннер — внутри ленты', () => {
    ...
    // ⚠ Изменено в #507: баннер уехал ВНУТРЬ ленты (врезка после третьей записи). Сверху он
    // загораживал работу на телефоне. Проверяем, что он отдан журналу слотом, а не стоит отдельным
    // блоком выше — иначе он снова окажется перед тем, ради чего человек пришёл.
    expect(t).toMatch(/<template #promo>/)
  })
