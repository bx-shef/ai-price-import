...
describe('#384: новая формулировка предупреждения о контрагенте', () => {
  ...
  it('обрезка не рвёт эмодзи пополам', () => {
    // `slice(0, 60)` режет по UTF-16 и разрывает суррогатную пару: половинка едет во все три
    // места, чат и дело показывают «», а портал, отвергающий такой JSON, молча теряет
    // best-effort сообщение — ровно на длинном чужом имени, ради которого кап и заведён.
    const w = supplierNotLinkedWarning('1', 'UNP', 'А'.repeat(59) + '😀ХВОСТ')
    expect(w, 'в тексте одинокая половина суррогатной пары').not.toMatch(/[\uD800-\uDBFF](?![\uDC00-\uDFFF])/)
    expect(w).not.toMatch(/(?<![\uD800-\uDBFF])[\uDC00-\uDFFF]/)
  })

  it('невидимые символы из документа вырезаются', () => {
    // BB-нейтрализация закрывает разметку, но правосторонний переворот меняет то, как имя ЧИТАЕТСЯ
    // — и в чате, и в карточке дела, и на экране. Шапка модуля называла инъекцию «уже
    // обезвреженной»: для BB это правда, для этих — нет.
    const w = supplierNotLinkedWarning('1', 'UNP', 'ООО \u202Eevil\u202C')
    expect(w).not.toMatch(/[\u202a-\u202e\u2066-\u2069\u200b-\u200d\ufeff\u061c]/)
    expect(w).toContain('ООО evil')
  })
