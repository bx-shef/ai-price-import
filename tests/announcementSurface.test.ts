 ...
 *
 * ⚠ ЧТО ЭТИ ПРОВЕРКИ НЕ ЛОВЯТ, сказано прямо, потому что «структурная проверка» звучит как «покрыто»
 * и создаёт ложную уверенность. Сравнение `screen === 'work'` сторожится ТЕКСТОМ: удаление условия
 * или переименование они заметят, а подмену значения на другое состояние того же перечисления
 * (`'launcher'`, `'setup'`) — НЕТ. Поведенчески это закрыть нечем, пока `/app` не монтируется в
 * тестах; знать об этом важнее, чем иметь проверку, о границах которой никто не помнит.
 */
