...
describe('проводка на странице настроек (#311)', () => {
  ...
  it('подсказка НЕ пишется в модель — она не значение по умолчанию', () => {
    // Решение владельца. Присвоение `savingsRate` из подсказки вернуло бы ровно #270.
    expect(PAGE).not.toMatch(/savingsRate(\.value)?\s*=\s*.*rateHint/)
  })
})
...
describe('подсказка под полем ставки (#311)', () => {
  ...
  it('подсказка НЕ выдаёт себя за рекомендацию', () => {
    ...
    expect(hourlyRateHint('RUB')!.text).toContain('не рекомендация')
