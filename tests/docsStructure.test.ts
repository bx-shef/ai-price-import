...
const ALLOWED_DOCS = [
  ...
  'site-terms.md',
  'site-privacy.md',
  // Шаблоны договорных документов — договор, счёт, акт, в двух юрисдикциях (#419, решение владельца
  // 2026-08-12). Пятый файл «наружу»: эти бумаги уходят клиенту и юристу целиком, и их место в
  // `PRICING.md` кончилось бы тем, что вместе с договором клиент получил бы наши ставки, оценки
  // часов и маржу по источникам документов. ⚠ Файл — ФОРМА условий, а не их источник: сами условия
  // живут в `PRICING.md` §7, и при расхождении прав он. ⚠ Плейсхолдеры `‹…›` здесь разрешены и
  // обязательны — шаблон без них означал бы намертво вписанные реквизиты; запрет на плейсхолдеры
  // касается ПУБЛИКУЕМЫХ документов (`PUBLISHED_LEGAL`), которые уходят читателю как есть.
  'contracts.md',
...
/**
 * Шаблоны договорных документов (#419): плейсхолдер, которого нет в чек-листе, — это плейсхолдер,
 * который уедет клиенту.
 *
 * ⚠ Гард заведён по разбору проверяющих 12.08.2026: §0.2 объявлен полным списком «что подставить
 * перед отправкой», а в тексте нашлось около двенадцати токенов мимо него — включая `‹ВАЛЮТА›` в
 * пункте о расчётах. Менеджер проходит по чек-листу, считает документ готовым и отправляет договор
 * со строкой «Валюта расчётов — ‹ВАЛЮТА›».
 *
 * ⚠ Проверка структурная и дешёвая: сверяются ИМЕНА токенов, а не их значения. Числа и формулировки
 * гардом не сторожатся намеренно — такой тест пришлось бы править при каждой правке текста.
 */
describe('шаблоны договоров: чек-лист плейсхолдеров полон (#419)', () => {
  const text = readFileSync(abs('docs/contracts.md'), 'utf8')
  const used = [...new Set([...text.matchAll(/‹[A-ZА-Я_0-9]+›/g)].map(m => m[0]))]
  // Чек-лист — весь раздел §0.2 целиком, а не первая ячейка строки: в одной строке таблицы
  // перечисляют по три родственных токена, и разбор «по ячейкам» пропустил бы второй и третий.
  const checklist = text.slice(text.indexOf('### 0.2.'), text.indexOf('### 0.3.'))
  const listed = new Set([...checklist.matchAll(/‹[A-ZА-Я_0-9]+›/g)].map(m => m[0]))

  it('токенов вообще много — иначе проверка ниже пуста и зелена', () => {
    expect(used.length).toBeGreaterThan(15)
  })

  it('каждый использованный плейсхолдер перечислен в §0.2', () => {
    const missing = used.filter(p => !listed.has(p))
    expect(missing, `нет в чек-листе §0.2: ${missing.join(', ')}`).toEqual([])
  })
})

/**
 * Разделы `PRICING.md`, названные в таблице «Откуда что взято», существуют.
 *
 * ⚠ Тот же класс, что у справки для ИБ (гард выше): документ уходит клиенту и юристу, ссылается на
 * номера разделов чужого файла, и переименование раздела делает таблицу источников бутафорией —
 * молча, при зелёном CI. Смысл ссылки машина не проверит, существование адреса — проверит.
 */
describe('шаблоны договоров ссылаются на существующие разделы PRICING', () => {
  const text = readFileSync(abs('docs/contracts.md'), 'utf8')
  const pricing = readFileSync(abs('docs/PRICING.md'), 'utf8')
  const refs = [...new Set([...text.matchAll(/`PRICING\.md` §(\d+(?:\.\d+)?)/g)].map(m => m[1]!))]

  it('ссылки на PRICING вообще есть', () => {
    expect(refs.length).toBeGreaterThan(3)
  })

  for (const n of refs) {
    it(`PRICING.md §${n} существует`, () => {
      const re = new RegExp(`^#{2,3} ${n.replace('.', '\\.')}\\.`, 'm')
      expect(pricing, `PRICING.md не содержит раздела ${n}`).toMatch(re)
    })
  }
})
...
describe('CLAUDE.md не растёт обратно', () => {
  ...
  //
  // ⚠ 74 000 → 78 000 в тот же день, и это НЕ отступление: разбор проверяющих потребовал вернуть в
  // файл четыре кросс-тенантных инварианта (сужение по владельцу, сверка `member_id`, проверка
  // домена) и обязанность сверять документ при правке гарда — после них запас снова стал 53 байта,
  // то есть ровно то состояние, из которого файл выводили. Потолок обязан оставлять место хотя бы
  // на несколько инвариантов вперёд: слишком тесный не разгружает, а заставляет молчать.
  const CLAUDE_MAX_BYTES = 78_000
  ...
  /**
   * Обещанные адреса существуют (#508, разбор проверяющих 11.08.2026).
   *
   * ⚠ Проверка «битых ссылок» смотрит только на СУЩЕСТВОВАНИЕ ФАЙЛА, и это дыра ровно там, где
   * теперь держится вся конструкция: `CLAUDE.md` больше не носит инварианты подсистем, а обещает,
   * что они лежат в README и в §15. Мутации проверяющего прошли незамеченными обе: переименуй
   * заголовок «Подсистемы» в README или «## 15.» в процессе — ссылка на файл останется рабочей, а
   * обещание станет пустым, и заметит это только живой читатель.
   */
  const PROMISED = [
    ['README.md', /^## Подсистемы/m],
    ['docs/PROCESS.md', /^## 15\. Инварианты подсистем/m],
    ['docs/PROCESS.md', /^### 15\.1\./m],
    ['docs/PROCESS.md', /^### 15\.2\./m]
  ] as const
  for (const [file, re] of PROMISED) {
    it(`${file}: раздел ${re.source} на месте — на него ссылается CLAUDE.md`, () => {
      expect(readFileSync(abs(file), 'utf8')).toMatch(re)
    })
  }

  /**
   * Каждый `tests/*.test.ts`, названный в `CLAUDE.md`, существует.
   *
   * ⚠ Смысл («гард утверждает то же, что документ») машина не проверит, а вот исчезновение опоры —
   * да: правило со ссылкой на переименованный или удалённый гард выглядит подкреплённым, хотя за
   * ним уже ничего нет. Дефект этого класса в проекте случался — `tests/importBusyLock.test.ts`
   * уехал вместе с чужим файлом (#489), а `CLAUDE.md` продолжал ссылаться на него как на живой.
   */
  it('гарды, названные в CLAUDE.md, существуют', () => {
    const text = readFileSync(abs('CLAUDE.md'), 'utf8')
    const named = [...new Set([...text.matchAll(/tests\/[\w/.-]+\.test\.ts/g)].map(m => m[0]!))]
    expect(named.length, 'ссылок на гарды не найдено — регулярка перестала совпадать').toBeGreaterThan(5)
    const missing = named.filter(f => !existsSync(abs(f)))
    expect(missing, `названы, но не существуют: ${missing.join(', ')}`).toEqual([])
  })
