...
describe('CLAUDE.md не растёт обратно', () => {
  // ⚠ 140 000 → 74 000 (11.08.2026, #508). Прежний потолок перестал разгружать и начал ПРОИЗВОДИТЬ
  // ДРЕЙФ: запаса осталось 52 байта, дописать инвариант было физически некуда, а разгрузить чужой
  // блок дорого — и самым дешёвым путём, проходящим CI, стало не трогать `CLAUDE.md` вовсе. Так
  // файл разошёлся с кодом дважды за сутки (баннер над журналом против гарда, который к тому
  // моменту требовал обратного; и второй источник документа для отзыва). Гард гарантировал «не
  // растёт», но не «верен», а ценно именно второе. Раскладка и инварианты подсистем уехали в
  // `README.md`, два самых больших свода — в `PROCESS.md` §15.
  const CLAUDE_MAX_BYTES = 74_000
