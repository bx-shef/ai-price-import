...
describe('shouldPrompt', () => {
  it('shows when there is no row yet (first-ever)', () => {
    expect(shouldPrompt(null, NOW, {}, OLD)).toBe(true)
  })

  it('shows when a row exists but was never actually prompted', () => {
    expect(shouldPrompt({ promptedAt: null, openedAt: null, reviewed: false }, NOW, {}, OLD)).toBe(true)
  })

  it('never shows once a review is confirmed', () => {
    expect(shouldPrompt({ promptedAt: daysAgo(999), openedAt: null, reviewed: true }, NOW, {}, OLD)).toBe(false)
  })

  it('suppresses while opened_at is set (awaiting manual verification)', () => {
    expect(shouldPrompt({ promptedAt: daysAgo(999), openedAt: daysAgo(1), reviewed: false }, NOW, {}, OLD)).toBe(false)
  })

  it('throttles: hidden within the re-prompt interval', () => {
    expect(shouldPrompt({ promptedAt: daysAgo(RATING_REPROMPT_DAYS - 1), openedAt: null, reviewed: false }, NOW, {}, OLD)).toBe(false)
  })

  it('shows again once the re-prompt interval has elapsed', () => {
    expect(shouldPrompt({ promptedAt: daysAgo(RATING_REPROMPT_DAYS), openedAt: null, reviewed: false }, NOW, {}, OLD)).toBe(true)
  })

  it('honours a custom re-prompt interval', () => {
    const st = { promptedAt: daysAgo(2), openedAt: null, reviewed: false }
    expect(shouldPrompt(st, NOW, { repromptDays: 1 }, OLD)).toBe(true)
    expect(shouldPrompt(st, NOW, { repromptDays: 3 }, OLD)).toBe(false)
  })
})
...
describe('#380: попап не раньше N суток после установки', () => {
  ...
  it('портал моложе порога попапа не получает', () => {
    ...
    expect(shouldPrompt(fresh, NOW, {}, daysAgo(RATING_MIN_INSTALL_AGE_DAYS - 1))).toBe(false)
    expect(shouldPrompt(null, NOW, {}, daysAgo(0))).toBe(false)
  })

  it('на N-е сутки появляется на первом же успешном импорте', () => {
    expect(shouldPrompt(fresh, NOW, {}, daysAgo(RATING_MIN_INSTALL_AGE_DAYS))).toBe(true)
    expect(shouldPrompt(null, NOW, {}, daysAgo(RATING_MIN_INSTALL_AGE_DAYS))).toBe(true)
  })
  ...
  it('возраст неизвестен → НЕ показываем', () => {
    ...
    expect(shouldPrompt(fresh, NOW, {}, null)).toBe(false)
    expect(shouldPrompt(fresh, NOW, {}, undefined)).toBe(false)
    expect(shouldPrompt(null, NOW, {}, null)).toBe(false)
  })
  ...
  it('возраст — ОТДЕЛЬНОЕ условие, а не замена прежним', () => {
    ...
    expect(shouldPrompt({ promptedAt: daysAgo(999), openedAt: null, reviewed: true }, NOW, {}, OLD)).toBe(false)
    expect(shouldPrompt({ promptedAt: daysAgo(999), openedAt: daysAgo(1), reviewed: false }, NOW, {}, OLD)).toBe(false)
    expect(shouldPrompt({ promptedAt: daysAgo(1), openedAt: null, reviewed: false }, NOW, {}, OLD)).toBe(false)
  })
  ...
  it('два срока — две независимые константы', () => {
    ...
    expect(shouldPrompt(st, NOW, { minInstallAgeDays: 1 }, daysAgo(2))).toBe(true)
    expect(shouldPrompt(st, NOW, { minInstallAgeDays: 10 }, daysAgo(2))).toBe(false)
    // Троттл при этом не трогается: свежепоказанный попап молчит на любом возрасте установки.
    expect(shouldPrompt({ promptedAt: daysAgo(1), openedAt: null, reviewed: false }, NOW, { minInstallAgeDays: 0 }, daysAgo(365))).toBe(false)
  })
})
