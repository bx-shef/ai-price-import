...
describe('#387: лендинг не обещает цену, которой не назначено', () => {
  ...
  it('слова о бесплатности самого приложения нет', () => {
    for (const t of [LANDING_HERO_NOTE, LANDING_WHY_SUBTITLE, LANDING_MARKET_PROMO.text]) {
      expect(t, t).not.toMatch(/бесплатн/i)
    }
  })

  it('и цены тоже нет — её ещё не назначили', () => {
    for (const t of [LANDING_HERO_NOTE, LANDING_WHY_SUBTITLE, LANDING_MARKET_PROMO.text]) {
      expect(t, t).not.toMatch(/\d+\s*(₽|руб|BYN|Br|₸)/i)
    }
  })
