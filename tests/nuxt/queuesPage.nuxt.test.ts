 ...
 *
 * Не дождались — ПАДАЕМ с внятным текстом. Раньше помощник молча выходил по исчерпании попыток, и
 * тест валился дальше на `expected undefined to be truthy`: причина (не дорисовалось за отпущенное
 * время) была не видна, а сам прогон стал плавающим. Бюджет заодно поднят с 0,6 до 3 секунд —
 * на холодном прогоне трёх запросов подряд его не хватало.
 */
const flush = async (w?: { text: () => string }, needle?: string) => {
  ...
  if (w && needle) throw new Error(`не дождались «${needle}» на странице`)
}
...
describe('Операторская консоль (#271)', () => {
  ...
  it('I: «Отзыв оставлен» требует подтверждения — состояние терминально, откат только через базу', async () => {
    const w = await mountSuspended(QueuesPage)
    // Ждём именно СТРОКУ портала, а не слова «Отзыв оставлен»: они есть и в пояснении над списком,
    // поэтому ожидание завершалось ещё до отрисовки строки — и кнопки не находилось (плавающий тест).
    await flush(w, 'показан, Маркет не открыл')
