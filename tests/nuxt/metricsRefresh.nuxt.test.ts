...
describe('#444: метрики пересчитываются после импорта', () => {
  it('обновление привязано к СНЯТИЮ блокировки, а не к каждому заданию', () => {
    ...
    // ⚠ Метрики теперь грузит сама карточка «Экономия» (#523), а страница лишь даёт повод их
    // перечитать — через `defineExpose({ reload })`. Условие «на снятие блокировки, а не на каждое
    // задание» от этого не изменилось, изменился адресат вызова.
    expect(page).toMatch(/watch\(busy,\s*\(now, was\)\s*=>\s*\{\s*if \(was && !now\) savingsCard\.value\?\.reload\(\{ silent: true \}\)/)
  })
