...
describe('errorKind', () => {
  ...
  it('strips non-token chars (a message-y code cannot leak free text/PII)', () => {
    expect(errorKind({ code: 'поставщик ООО Ромашка цена 1840' })).toBe('1840') // cyrillic/spaces stripped
...
describe('withDependencySpan', () => {
  ...
  it('rethrows the original error (never swallows)', async () => {
    ...
    await expect(withDependencySpan({ system: 'bitrix24', operation: 'crm.product.add' }, async () => {
      throw err
    })).rejects.toBe(err)
  })
})
...
describe('withSpan', () => {
  it('returns the fn result and accepts a finalize hook', async () => {
    const r = await withSpan('crm-sync', { 'job.queue': 'crm-sync' }, async () => ({ rowCount: 3 }), res => ({ 'proc.lines': res.rowCount }))
    expect(r).toEqual({ rowCount: 3 })
  })
  it('rethrows on error', async () => {
    await expect(withSpan('crm-sync', {}, async () => {
      throw new Error('x')
    })).rejects.toThrow('x')
  })
})
