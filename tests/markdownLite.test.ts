...
describe('#473 п.6: текст объявления идёт в ЧУЖУЮ CRM', () => {
  ...
  it('кавычка в адресе не разрывает атрибут', () => {
    // ⚠ Безопасность этого случая ОБЕСПЕЧЕНА порядком (вся строка экранируется ДО сборки атрибута),
    // но до сих пор доказывалась только чтением кода. Для канала, который рисует текст во всех
    // клиентских CRM, «доказывается чтением» — недостаточное основание: порядок можно переставить,
    // и тест обязан это заметить.
    const out = render('[x](https://a" onmouseover="alert(1))')
    const tags = out.match(/<[^>]+>/g) ?? []
    for (const t of tags) expect(t, `в теге появился обработчик: ${t}`).not.toMatch(/\son\w+=/i)
    expect(out).not.toContain('onmouseover="alert')
  })

  it('якорь новой вкладкой НЕ открывается', () => {
    // Переход внутри того же документа фрейм никуда не уводит, и вкладку для него плодить незачем.
    const out = render('[к разделу](#part)')
