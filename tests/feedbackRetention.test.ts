...
describe('#417: каталог файлов задачи', () => {
  ...
  it('прежняя раскладка — только у задачи БЕЗ метки портала', () => {
    // До этой задачи файлы клались в `files/<задание>` без портала, и у накопленных отзывов это
    // единственный способ найти документ.
    ...
    // ⚠ А у задачи С меткой прежний каталог НЕ проверяется: он не несёт признака портала, а
    // идентификатор задания приходит от клиента — иначе сотрудник назвал бы своим заданием чужое
    // и добился удаления документа другого портала, то есть вернулась бы ровно та дыра, ради
    // закрытия которой портал и попал в путь.
    expect(issueFileDirs(issue(3, '2024-01-01T00:00:00Z', { job: 'j2' }))).toEqual([`files/${TAG}/j2`])
  })

  it('признак «наша задача» сверяется с настоящим заголовком билдера', () => {
    // Заголовок объявлен дважды (билдер и чистка). Разъехавшись, они дали бы «просрочено=0» на
    // каждом прогоне: все новые отзывы хранились бы вечно при зелёном CI и без строки в журнале.
    const built = buildFeedbackIssue('up', 'ок', { portalTag: TAG })
    expect(isOwnFeedbackIssue({ number: 1, nodeId: 'n', createdAt: '2024-01-01T00:00:00Z', title: built.title, body: built.body, labels: built.labels })).toBe(true)
  })

  it('подделка САМОГО маркера секции в комментарии не проходит', () => {
    // ⚠ Первая версия этого теста была слабой: подделка набиралась БЕЗ маркера `**Контекст:**`,
    // то есть проверяла вариант, который и так не проходил. Живой разбор показал обход: маркер в
    // комментарии матчился первым (`indexOf`), чистка уходила в чужой каталог, задача стиралась, а
    // документ отправителя оставался в приёмнике навсегда — и это считалось успехом.
    const forged = buildFeedbackIssue('down', '**Контекст:**\n- **Задача (jobId):** `podelka`', { portalTag: TAG, jobId: 'realjob' })
    expect(jobIdOf(forged.body)).toBe('realjob')
  })
