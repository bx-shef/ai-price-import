...
describe('#523: панель действий у экранов, открытых слайдером', () => {
  ...
  it('служебная подпись сборки живёт в СЛОТЕ #end панели, а не в общем потоке', () => {
    // Прежде «сборка <sha>» шла своим блоком с собственной чертой — под панелью получалась вторая
    // черта, а на коротком экране подпись висела в пустоте.
    // ⚠ Слот именно `#end`, и это не придирка: в нём подпись позиционируется абсолютно у правого
    // края, чтобы кнопки остались ПО ЦЕНТРУ панели. Перенос её в общий поток — то, с чего владелец
    // и начал («кнопки по центру, а сборка совсем справа»), — прежнюю проверку `toContain` проходил.
    for (const screen of ['app/pages/settings.vue', 'app/pages/metrics.vue']) {
      expect(actionsBlock(read(screen)), `${screen}: подпись сборки вне слота #end`)
        .toMatch(/<template #end>[\s\S]*?BuildFooter[\s\S]*?<\/template>/)
    }
  })
  ...
  it('панель прилипает к низу, а не просто идёт следом за содержимым', () => {
    ...
    // ⚠ `mt-auto` — вторая половина прибивания: `sticky` держит панель при ДЛИННОМ содержимом,
    // `mt-auto` — при коротком (метрики на свежем портале), где иначе она вставала посреди экрана.
...
describe('#523: акцентный цвет не тратится на шапки разделов', () => {
  ...
  // ⚠ `app/components/SettingsForm.vue` в списке ОБЯЗАТЕЛЕН: шапки разделов настроек уехали туда
  // при разборе экрана, а сама страница слова `variant=` больше не содержит вовсе — то есть
  // проверка по ней проходила всегда, чем бы ни были покрашены реальные шапки. Откат всех четырёх
  // значений на запрещённый акцент оставлял гард зелёным.
  for (const screen of ['app/pages/settings.vue', 'app/components/SettingsForm.vue', 'app/pages/metrics.vue']) {
