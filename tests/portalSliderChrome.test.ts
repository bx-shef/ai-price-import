...
describe('#523: панель действий у экранов, открытых слайдером', () => {
  ...
  it('служебная подпись сборки живёт В панели, а не отдельной строкой под ней', () => {
    ...
    for (const screen of ['app/pages/settings.vue', 'app/pages/metrics.vue']) {
      expect(actionsBlock(read(screen)), `${screen}: подпись сборки вне панели`).toContain('BuildFooter')
    }
  })
