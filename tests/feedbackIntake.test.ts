...
describe('роут действительно спрашивает гейт', () => {
  ...
  it('байты из тела — ТОЛЬКО когда СЕРВЕР НЕ НАШЁЛ ДЕЛА (#461 + #506 п.3)', () => {
    // ⚠ Правило СУЖЕНО, а не отменено, и разница несущая. #461 убрал байты из тела, потому что
    // страница была ЕДИНСТВЕННЫМ источником документа: перезагрузил вкладку — и отзыв уходил без
    // вложения, то есть без того, ради чего он и нужен. Это по-прежнему запрещено.
    //
    // Но у импорта, упавшего НА ИЗВЛЕЧЕНИИ, дела не существует вовсе: до CRM он не дошёл, вложение
    // взять неоткуда, и документ теряется ровно в том случае, когда без него воспроизвести нечего.
    //
    // ⚠ Граница — вердикт СЕРВЕРА (`miss === 'no-activity'`), а не `!fileUrl`: первая редакция
    // проверяла второе, и это пропускало почти всё. `!fileUrl` истинен, когда дело есть, но не
    // скачалось; когда исчерпан предел приёмника; и — главное — когда `jobId` вовсе не прислали,
    // то есть путь через дело даже не запускался. Условие выглядело как граница и ею не было.
    expect(route).not.toContain('parseClientFile')
    // Решение вынесено в чистую `shouldAcceptUploadedDoc` и покрыто ПОВЕДЕНИЕМ
    // (`tests/feedbackUpload.test.ts`): «дело есть, но не прочиталось» и «путь не запускался» байтов
    // не пропускают. Здесь сторожим лишь то, что роут судит ею, а не своим условием.
    expect(route, 'фолбэк идёт через общее решение').toMatch(/shouldAcceptUploadedDoc\(\{ attachFile, fileUrl, miss: attachMiss/)
    expect(route, 'безусловного приёма байт быть не должно').not.toMatch(/attachFile && raw\?\.fileUpload/)
  })
