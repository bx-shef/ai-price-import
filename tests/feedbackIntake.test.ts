...
describe('роут действительно спрашивает гейт', () => {
  ...
  it('байты из тела — ТОЛЬКО когда дела нет (#461 + #506 п.3)', () => {
    ...
    // Здесь страница не подменяет проверенный источник — она остаётся единственным возможным.
    //
    // Граница закреплена ТЕКСТОМ УСЛОВИЯ: байты берутся только при `!fileUrl`, то есть когда дело
    // ничего не дало. Снимут условие — гард покраснеет, и это правильный исход: возврат к «страница
    // всегда везёт байты» обязан быть замечен.
    expect(route).not.toContain('parseClientFile')
    expect(route, 'дело остаётся ОСНОВНЫМ источником').toMatch(/attachFile && !fileUrl && raw\?\.fileUpload/)
