...
- `server/` — Nitro backend: `api` / `routes` (не-`/api` публичные роуты) / `utils` (чистые с DI) /
  ...
  - **Алертинг очередей + телеграм-канал** ([`docs/BACKLOG.md`](docs/BACKLOG.md) §1): на cron-инстансе
    ...
    «восстановилось»). Схлопнуто в `evaluateQueueHealth`: все очереди нечитаемы ⇒ ОДИН эпизод
    (`ALL_QUEUES='*'`, текст про Redis без имени очереди — «очередь «*»» читалось бы как поломка
    самого сообщения). Схлопываем **только** когда нечитаемы ВСЕ: частичная авария имеет свою
    причину, и имя выпавшей очереди — единственное, что там помогает. Гард на `queues.length > 0`
    обязателен: `[].every(…)` истинно, и пустой замер объявил бы полную аварию.
