...
> Маркета), [`docs/privacy-policy.md`](docs/privacy-policy.md) и [`docs/eula.md`](docs/eula.md)
> (юристу и на публикацию, #297) и [`docs/PRICING.md`](docs/PRICING.md) (модель заработка +
...
- **Юридические документы: файл — источник, страница — рендер** (#297, вариант В): `docs/eula.md` и
  `docs/privacy-policy.md` рендерятся страницами `/eula` и `/privacy` через `app/utils/markdownLite.ts`
  ...
  http(s). Это единственное место в проекте с `v-html`. Обе страницы **индексируемы** — исключение из
  правила «индексируется только лендинг» именованное, в `tests/seoFiles.test.ts`.
