...
- `server/` — Nitro backend: `api` / `utils` (чистые с DI) / `queue` (BullMQ) / `db` / `plugins` / `agent`.
  ...
  - **crm-sync — запись документа в сделку** (`queue/crmSyncCore.runCrmSync`, чистое ядро с DI, тесты;
    ...
    - **Настраиваемое дело в таймлайне** (`configurableActivity.ts`+`liveDeps.writeActivity`, `crm.activity.configurable.add`,
      ...
      (в компании → открыть сделку, в сделке → открыть компанию); **нет компании → на деле сделки кнопки нет**
      (открывать нечего). В теле — счётчики + блок **«Проблемы (N):»** (warnings импорта). Кнопка «Исходный файл»
      ведёт на архивную копию на Диске **сконструированным** URL `/docs/file/<путь>?IFRAME=Y&IFRAME_TYPE=SIDE_SLIDER`
      (`disk.commonDiskFileUrl` — не API-`DETAIL_URL`, тот не открывал файл; сегменты `encodeURIComponent`+скобки).
      Внешние поля BB-нейтрализованы. Привязка к компании (ownerTypeId=4) **live-verified**. Best-effort: сбой дела
      импорт не роняет.
