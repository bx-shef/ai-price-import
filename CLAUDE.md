...
- `prompts/` — **промпт извлечения** (`extract.ts`, `buildExtractionPrompt`): единственное место, где
  ...
  **Текст замерен, а не написан на глаз** (#337): в шапке файла лежат числа A/B против `origin/main`
  ...
  нумерацию правил (она нарочно идёт 1,2,3,8,7,4,5,6). Перезамер — `pnpm ab:prompt`. Замер в CI не
...
pnpm ab:prompt --dir <корпус> --runs 2   # A/B промпта извлечения против origin/main (#337).
                  # Корпус — .txt реальных документов ВНЕ репозитория (клиентские данные!), готовить
                  # теми же раннерами, что у воркера. Дедуп по содержимому, сырой результат в --out
                  # (по умолчанию ab-prompt.json — гитигнорится), парный McNemar в конце.
