...
- `server/` — Nitro backend: `api` / `routes` (не-`/api` публичные роуты) / `utils` (чистые с DI) /
  ...
  - **SEO лендинга** (#292): share/SEO-мета живёт в `app/pages/index.vue`, **не** в корневом `app.vue` —
    ...
    **Сама картинка `public/og.png` — закоммиченный бинарник, который собирает РУЧНОЙ `pnpm og`**
    ...
    после #325 висел прежний подзаголовок и снятая «1С»). Теперь генератор пишет рядом
    `public/og.stamp.json` — хэш своих входов (`LANDING_TITLE`/`LANDING_SUBTITLE`/`LANDING_FORMATS` +
    разметка бейджа), а `tests/ogStamp.test.ts` пересчитывает его: правка копирайта без `pnpm og`
    роняет CI. **Не по mtime** — git времена не хранит, на свежем клоне (каждый прогон CI) всё
    одинаково «новое». Подвал карточки собирается из `LANDING_FORMATS` (третья рукописная копия
    списка форматов убрана), надзаголовок — тот же бейдж, что на лендинге: `app/config/brandBadge.ts`
    держит токены **обычными CSS-значениями** (Chromium в генераторе не знает ни Vue, ни Tailwind), а
    `tests/brandBadge.test.ts` пришивает их к классам `BrandBadge.vue` — правка одной стороны краснеет.
