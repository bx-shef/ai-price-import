...
- `server/` — Nitro backend: `api` / `routes` (не-`/api` публичные роуты) / `utils` (чистые с DI) /
  ...
  - **crm-sync — запись документа в сделку** (`queue/crmSyncCore.runCrmSync`, чистое ядро с DI, тесты;
    ...
    - **Подбор товара** (`productLookup.findProduct`): **приоритет торговых предложений (SKU/ТП)** —
      ...
      **`productName` уходит ТОЛЬКО у ненайденной позиции** (#348): поле необязательно при заданном
      ...
      как `productId`), но подстановка на нём вживую **не переверифицирована** — а это приоритетный путь
      подбора. Название из документа остаётся в уведомлении и в деле таймлайна (они читают `DocumentItem`,
  ...
  - **Сообщения в чат — от имени приложения** (`chatBot.ts` + `chatNotify.sendChatMessage`, #316):
    ...
    **Старый путь — обязательный фолбэк, не запас:** на бесплатном тарифе боты недоступны
    ...
    деградация немая. ⚠ **аватар не шлём** — нет иконки (#298) и точное имя поля в `properties` в
    справочнике не названо. ⚠ **вживую не проверено.**
  ...
  - **Трекинг задания импорта — Redis+TTL, НЕ Postgres** (`utils/jobStore.ts` + `utils/jobStoreRedis.ts`):
    **исходный файл НЕ хранится** (#349, откат #200): воркер удаляет байты сразу после извлечения текста, свип
    ...
    своей копии нет): вложение это «что прислал сотрудник», а не «что мы разбирали». ⚠ Лимитеры in-memory —
    гейт перед масштабированием HTTP-роли (#353); глобального предела на репозиторий-приёмник нет (#354).
