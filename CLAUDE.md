...
- **Оболочка публичных страниц — один источник фона И текста** (`app/config/landingShell.ts`, #368):
  ...
  открывает модератор Маркета**. ⚠ Токены b24ui (`var(--ui-color-base-*)`) тут **не работают**: у
  публичных страниц нет обёртки `B24App` (в `app/layouts/` один `clear.vue`, и он для in-portal), тема
  не разворачивается, и переменная резолвится в остаток светлой темы — именно они и стояли в
  `LegalDocument.vue`. Поэтому обычный Tailwind, а цвета собраны в `SHELL_COLORS` **именами палитры**:
  `tests/landingShell.test.ts` читает их значения из самой темы Tailwind и **считает контраст по WCAG**
  числом. Гард сразу окупился — задуманный «неброский» `slate-700` у рамок таблицы дал 1.99:1, то есть
  таблица в политике была бы слипшимся абзацем («неброско» и «невидимо» на тёмном фоне на глаз
  неразличимы). Структурный гард вырезает строки `import` **перед** поиском: первая версия находила имя
  в импорте и пропускала мутацию «убрать класс с корневого элемента» — сторожила импорт, а не применение.
