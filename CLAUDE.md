...
- `server/` — Nitro backend: `api` / `routes` (не-`/api` публичные роуты) / `utils` (чистые с DI) /
  ...
  - **SEO лендинга** (#292): share/SEO-мета живёт в `app/pages/index.vue`, **не** в корневом `app.vue` —
    ...
    `/robots.txt` + `/sitemap.xml` — **роуты** (`server/routes/*.ts` + чистые билдеры
    `utils/seoFiles.ts`, тесты), не статика в `public/`: обоим нужен абсолютный хост, который различается
    по деплою. **Без суффикса `.get`** — h3 не подменяет HEAD на GET, а краулерные инструменты ходят HEAD.
    `siteUrl`/`buildDate` читаются в рантайме ⇒ обе переменные обязаны стоять и на **рантайм-стадии**
    образа (`ENV` не переходит через `FROM` — тот же капкан, что с `COMMIT_SHA`).
