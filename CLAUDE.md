...
- `server/` — Nitro backend: `api` / `routes` (не-`/api` публичные роуты) / `utils` (чистые с DI) /
  ...
  - **Трекинг задания импорта — Redis+TTL, НЕ Postgres** (`utils/jobStore.ts` + `utils/jobStoreRedis.ts`):
    **исходный файл живёт до истечения TTL задания** (#200): воркер больше НЕ удаляет байты сразу после извлечения — иначе
    у 👎 на «документ не распознан» нечего приложить (там нет ни сущности в CRM, ни архива на Диске). Окно = `JOB_TTL_MS`
    (та же ручка, что у записи задания — два числа разъехались бы), чистит часовой `retentionSweep`→`sweepOldUploads`;
    удаление портала сносит файлы сразу (`purgePortalFiles`). Отзыв берёт **локальную копию**, Диск — фолбэк.
    ...
    **Механика прогона** (`ImportStaging`): цель **одна на пачку**, «Импортировать» шлёт файлы пачкой
    ...
    задания — текст говорит это явно. Статус опрашивается **по id**
