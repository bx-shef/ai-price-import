...
- **Альтернативный таргет деплоя — Битрикс24 Вайбкод Black Hole** (закрытый Bitrix-Cloud VM по REST,
  ...
  ⚠ **`frame-ancestors` покрывает только `*.bitrix24.com/.ru/.by/.eu/.kz`** (#323): портал на `.tech`,
  `.de`, `.es`, `.fr` и любой **коробочный** на своём домене получит в `/app` **белый экран** — браузер
  запретит встраивание, а в интерфейсе это выглядит как «приложение не работает» (ошибка только в
  консоли). Это не дыра, а функциональный отказ; правка обязана идти **одновременно** в `nginx.conf` и
  `edgeSecurity.ts`, иначе тест паритета краснеет. **За nginx флаг НЕ ставим**
