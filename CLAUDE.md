...
- **Веб-аналитики на сайте НЕТ** (решение владельца 2026-08-04, #297): счётчик собирает IP и cookie
  посетителя, которого никто не поручал обрабатывать, — издатель становится владельцем ПДн по 99-З и
  оператором по 152-ФЗ, то есть теряет позицию «мы обработчик», на которой стоит весь остальной
  пакет. Заявки считаются в самой CRM Битрикс24, отдельный счётчик не нужен. `metrikaId` пуст,
  `useMetrikaGoal` без него — no-op; гард `tests/noWebAnalytics.test.ts` закрывает все четыре
  способа вернуть счётчик (id в конфиге, инлайн-сниппет, скрипт в `public/`, хост в CSP — он
  появляется раньше самого счётчика). Тексты приведены в соответствие: раздел 5 Политики сайта
  констатирует отсутствие аналитики (правка внесена по поручению владельца, юристу показать при
  ближайшей вычитке). Подключение возможно, но **только после** новой редакции Политики сайта:
  обратный порядок = месяц обработки вопреки опубликованному документу.
