...
- `server/` — Nitro backend: `api` / `routes` (не-`/api` публичные роуты) / `utils` (чистые с DI) /
  ...
  - **Сообщения в чат — от имени приложения** (`chatBot.ts` + `chatNotify.sendChatMessage`, #316):
    ...
    **Старый путь — обязательный фолбэк, не запас:** на бесплатном тарифе боты недоступны
    ...
    деградация немая. **Аватар шлём** (#298) — `properties.avatar`, голый base64 без
    ...
    симптома #316). Поэтому на любой отказ идёт один повтор **без аватара**. ⚠ **вживую не проверено.**
