  ...
  - **crm-sync — запись документа в сделку** (`queue/crmSyncCore.runCrmSync`, чистое ядро с DI, тесты;
    ...
    - **Цена/НДС/итог** (`app/utils/pricing.ts`): НДС считаем **построчно** (округление раз на строку:
      ...
      **Строка в портал — всегда валовая цена** (#302, live-verified 2026-08-01): `crm.item.productrow.*`
      ...
      net→gross **сам** и БЕЗ округления до копейки (6 знаков: 1,032, не 1,03 — иначе ×10 000 = 10 300, тот же
      дефект с другой стороны; портал хранит ≥6 знаков как есть), `taxIncluded` теперь всегда `'Y'`.
      `pnpm live:crm` читает строки **обратно** и сверяет Σ price×qty с суммой сущности — прежняя проверка
      печатала `opportunity`, который мы сами и проставили.
